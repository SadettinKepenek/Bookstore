...
public class ReduceStockTests
{
    ...
    [Fact]
    public async Task ReduceStockAsync_ValidBookAndSufficientStock_ShouldReduceStock()
    {
        // Arrange
        ...
        // Act
        await _bookService.ReduceStockAsync(bookId, 3, CancellationToken.None);

        // Assert
        book.Stock.Should().Be(2);
    ...
    [Fact]
    public async Task ReduceStockAsync_BookNotFound_ShouldThrowNotFoundException()
    {
        // Arrange
        var bookId = Guid.NewGuid();
        ...
        // Act
        var action = async () => await _bookService.ReduceStockAsync(bookId, 3, CancellationToken.None);
